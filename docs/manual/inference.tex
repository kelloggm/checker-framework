...
\item[DLJC-ARGS]
  ...
  modification.  One argument is usually required: \<-\relax-checker>, which indicates
  what type-checker(s) to run (in the format described in Section~\ref{shorthand-for-checkers}).
  If no \<-\relax-checker> argument is supplied, then the standard annotation processor discovery
  mechanism is used by javac; see the \ahreforurl{https://github.com/kelloggm/do-like-javac}{documentation of do-like-javac}
  for details.
  The \ahreforurl{https://github.com/kelloggm/do-like-javac}{documentation of do-like-javac} also
